...
to the magnetic field on Earth.  This version uses a
...
$\nabla \cdot B = 0$, which may be violated due to numerical error. Various MHD models 
use different techniques to limit $\nabla \cdot B$. 
...
point.  The Biot-Savart calculations use the magnetosphere 
current densities. The current densities are specfied in the 
