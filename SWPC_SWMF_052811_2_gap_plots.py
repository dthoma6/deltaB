...
def surf_point(info, surface_locations, n_steps=None):
    ...
    # Read my data
    pklname = 'dB_bs_gap-' + surface_location + '.pkl'
    ...
    #
    bs_fac.columns =['north_bs','east_bs','down_bs','mag_bs']
    dBFac.columns =['north_db','east_db','down_db','mag_db']
    df_all = bs_fac.merge( dBFac, left_index=True, right_index=True)
    ...
    df_all['db mine ratio N'] = df_all['north_db'] / df_all['Bn']
    # df_all.plot( y='db mine ratio N', label='N ratio')
    
    df_all['db mine ratio E'] = df_all['east_db'] / df_all['Be']
    # df_all.plot( y='db mine ratio E', label='E ratio')
    
    df_all['db mine ratio D'] = df_all['down_db'] / df_all['Bd']
    # df_all.plot( y='db mine ratio D', label='D ratio')
    
    df_all['db mine ratio mag'] = df_all['mag_db'] / df_all['mag']
    # df_all.plot( y='db mine ratio mag', label= 'mag ratio')
    ...
    ax = df_all.plot( y='mag_bs', label='|B| Gary')
    df_all.plot(ax = ax, y='mag_db', label='|B| CCMC')
    df_all.plot( ax=ax, y='mag', label='|B| Dean')
    ax.set_ylabel( r'$|B|$' )
 
    ax = df_all.plot( y='Bn', label='North Dean')
    ax.set_ylabel( r'$B_N$' )
     
    ax = df_all.plot( y='Be', label='East Dean')
    ax.set_ylabel( r'$B_E$' )
    
    ax = df_all.plot( y='Bd', label='Down Dean')
    ax.set_ylabel( r'$B_D$' )
 
    ax = df_all.plot( y='mag', label='|B| Dean')
    ax.set_ylabel( r'$|B|$' )
    ...
    print('Mean CCMC-Gary |B|: ', df_all['db-bs mag'].mean())
    print('Mean CCMC-Dean |B|: ', df_all['db-mine mag'].mean())
    print('Mean Gary-Dean |B|: ', df_all['bs-mine mag'].mean())
    print('db mine ratio N: ', df_all['db mine ratio N'].mean())
    print('db mine ratio E: ', df_all['db mine ratio E'].mean())
    print('db mine ratio D: ', df_all['db mine ratio D'].mean())
    print('db mine ratio mag: ', df_all['db mine ratio mag'].mean())
