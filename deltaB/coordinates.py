...
USE_TRANSFORMS = False     # When false, do not perform coordinate tranformations
                           # transform(v, time, csys_in, csys_out, ctype_in='car', ctype_out='car', lib='spacepy')
                           # will just return v
                           
USE_NED = True             # When false, do not perform decompostion into North-East-Down components
                           # get_NED_components(v_cart, x_cart) will just return v_cart

if not USE_TRANSFORMS or not USE_NED:
    import logging
import numpy as np
...
else: #USE_TRANFORMS is False
    def transform(v, time, csys_in, csys_out, ctype_in='car', ctype_out='car', lib='spacepy'):
        logging.info('WARNING: USE_TRANSFORMS is False, check options')
        return v
...
if USE_NED:
    ...
    def get_NED_components(v_cart, x_cart):
        logging.info('WARNING: USE_NED is False, check options')
        return v_cart
