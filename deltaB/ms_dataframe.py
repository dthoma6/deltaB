...
def convert_mhd_to_dataframe(mhd, region=None):
    """Process data in MHD file to create dataframe.  
    
    Inputs:
        mhd = MHD data stored in a named tuple, see BATSRUS_data.py,
            OpenGGCM_data.py, etc. for structure
    ...
    Outputs:
        df = dataframe containing data from MHD file plus additional calculated
            parameters
    """

    # Extract data from BATSRUS
    var_dict = dict(mhd.varidx)

    df = pd.DataFrame()

    df['x'] = mhd.data_arr[:, var_dict['x']][:]
    df['y'] = mhd.data_arr[:, var_dict['y']][:]
    df['z'] = mhd.data_arr[:, var_dict['z']][:]

    df['bx'] = mhd.data_arr[:, var_dict['bx']][:]
    df['by'] = mhd.data_arr[:, var_dict['by']][:]
    df['bz'] = mhd.data_arr[:, var_dict['bz']][:]

    df['jx'] = mhd.data_arr[:, var_dict['jx']][:]
    df['jy'] = mhd.data_arr[:, var_dict['jy']][:]
    df['jz'] = mhd.data_arr[:, var_dict['jz']][:]

    df['ux'] = mhd.data_arr[:, var_dict['ux']][:]
    df['uy'] = mhd.data_arr[:, var_dict['uy']][:]
    df['uz'] = mhd.data_arr[:, var_dict['uz']][:]

    df['p'] = mhd.data_arr[:, var_dict['p']][:]
    df['rho'] = mhd.data_arr[:, var_dict['rho']][:]
    df['measure'] = mhd.data_arr[:, var_dict['measure']][:]
    ...
    df = df.drop(df[df['r0'] < mhd.rCurrents].index)
    
    return df

def create_deltaB_spherical_dataframe(df):
    """Convert the dataframe with MHD data to a dataframe that includes
    spherical coordinates breakdown of current density 
    
    Inputs:
        df = dataframe with MHD and other calculated quantities from
            convert_*_to_dataframe
                        
    Outputs:
        df = dataframe with all the df data plus spherical coordinates
    """

    logging.info('Creating spherical coordinate MHD dataframe...')
    
    # Verify that we have something that looks like a MHD dataframe
...
def create_deltaB_biotsavart_dataframe(df, X):
    """Convert the dataframe with MHD data to a dataframe that uses 
    Biot-Savart Law to calculate delta B in each MHD cell outside of 
    rCurrents.  Points inside rCurrents were dropped in convert_*_to_dataframe
    ...
    logging.info('Creating deltaB >rCurrents MHD dataframe...')
    ...
    ##########################################################
    # Note same units for OpenGGCM results
    ##########################################################
...
def create_deltaB_biotsavart_spherical_dataframe(df, X):
    """Convert the dataframe with MHD data to a dataframe that uses 
    Biot-Savart Law to calculate delta dB in spherical coordinates.  delta B
    is determined in each MHD cell outside of rCurrents.  Cells inside of
    ...
    logging.info('Creating deltaB spherical coordinates MHD dataframe...')

    # Verify that MHD dataframe has spherical elements and it hasn't already
    # been converted
...
def create_cumulative_sum_dataframe(df):
    """Convert the dataframe with MHD data to a dataframe that provides a 
    ...
    logging.info('Creating cumulative sum MHD dataframe...')
...
def create_jrtp_cdf_dataframes(df):
    """Use the dataframe with MHD dataframe to develop jr, jtheta, jphi
    cummulative distribution functions (CDFs).

    Inputs:
        df = dataframe with MHD and other calculated quantities
...
def create_jpp_cdf_dataframes(df):
    """Use the dataframe with MHD dataframe to develop jparallel and
