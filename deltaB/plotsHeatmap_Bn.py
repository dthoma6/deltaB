...
def loop_heatmap_ms(info, times, nlat, nlong):
    ...
    for p in range(len(times)):
        ...
        noonsm = coord.Coords([[1,0,0]], 'SM', 'car', use_irbem=False)
...
def loop_heatmap_ms_by_region(info, times, nlat, nlong, deltamp, deltabs, thicknessns, nearradius, 
    ...
    for p in range(len(times)):
        ...
        create_directory(info['dir_derived'], 'heatmaps')
        pklname = basename + '.' + params + '.ms-region-heatmap.pkl'
        df.to_pickle( os.path.join( info['dir_derived'], 'heatmaps', pklname) )
...
def plot_heatmap_ms_by_region( info, times, vmin, vmax, nlat, nlong, deltamp, deltabs, 
...
def loop_heatmap_gap(info, times, nlat, nlong, nTheta=30, nPhi=30, nR=30):
