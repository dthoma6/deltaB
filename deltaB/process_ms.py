...
def calc_ms_b(X, timeISO, df):
    ...
    Inputs:
        ...
        timeISO = time (in ISO format) for MHD file
    ...
    df = create_deltaB_biotsavart_dataframe(df, X)
...
def calc_ms_b_paraperp_sub( dBx, dBy, dBz, 
    ...
    Inputs:
        dBx, et al. = x,y,z components of magnetic field at each point in
            MHD grid stored in numpy arrays.
...
def calc_ms_b_paraperp(XGSM, timeISO, df, northonly=True):
    """Use Biot-Savart to determine the magnetic field (in North-East-Down 
    coordinates) at point XGSM.  Biot-Savart caclculation uses magnetosphere 
    current density.  Bio-Savart integration from rCurrents to max range of
    MHD grid.  This routine adds a breakdown of the B field contributions
    ...
    Inputs:
        ...
        northonly = Boolean, True only return north component of B fields,
            False return north, east and adown components
    ...
    df1 = create_deltaB_biotsavart_dataframe(df, XGSM)
    ...
    if northonly:
        ...
        return Bn, Be, Bd, Bparan, Bparae, Bparad, \
                Bperpn, Bperpe, Bperpd, \
                Bperpphin, Bperpphie, Bperpphid, \
                Bperpresn, Bperprese, Bperpresd, \
                BSM[0], BSM[1], BSM[2]
...
def loop_ms_b(info, point, reduce, deltahr=None, maxcores=20, deltaBlist=False):
    ...
    uses magnetosphere current density as defined in MHD files
    ...
    def wrap_ms( i, times, deltahr, XGEO, info ):
        ...
        logging.info(f'Calculate magnetosphere dB for... {base}')
        ...
        # MHD data (originally based on GSM coordinates being used in BATSRUS)
        ...
        Bn, Be, Bd, Bparan, Bparae, Bparad, Bperpn, Bperpe, Bperpd, \
                Bperpphin, Bperpphie, Bperpphid, \
                Bperpresn, Bperprese, Bperpresd, \
                Bx, By, Bz = calc_ms_b_paraperp(X, timeISO, df, northonly=False)
    ...
    df = pd.DataFrame( data={'Bn': Bn, 'Be': Be, 'Bd': Bd,
                'Bparan': Bparan, 'Bparae': Bparae, 'Bparad': Bparad, 
                'Bperpn': Bperpn, 'Bperpe': Bperpe, 'Bperpd': Bperpd, 
                'Bperpphin': Bperpphin, 'Bperpphie': Bperpphie, 'Bperpphid': Bperpphid, 
                'Bperpresn': Bperpresn, 'Bperprese': Bperprese, 'Bperpresd': Bperpresd, 
                'Bx': Bx, 'By': By, 'Bz': Bz, 
